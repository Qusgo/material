...
function addBody(worldOrBody,maybeBody){
  ...
  if(!canPlaceBody(body)){
    reportBodyPlacementStatus('Dynamic stones cannot overlap or start outside the canvas');
    return false;
  }
  clearGridUnderBody(body);
  bodies.push(body);
...
function rebuildBodyMask(world){
  ...
  for(const b of bodies){
    ...
    for(let r=minR;r<=maxR;r++)for(let c=minC;c<=maxC;c++){
      ...
      if(bodyContainsPoint(b,p.x,p.y))bodyMask[idx(c,r)]=1;
    }
  }
}
