...
function syncButtons(){const activeTool=currentTool(),activeSelected=currentSelectedKey();document.querySelectorAll('[data-tool]').forEach(b=>b.classList.toggle('active',b.dataset.tool===activeTool));document.querySelectorAll('[data-material]').forEach(b=>b.classList.toggle('active',b.dataset.material===activeSelected));playBtn.textContent=appContext.running?'Pause':'Play';if(debugBasinsBtn){debugBasinsBtn.classList.toggle('active',appContext.debugBasins);debugBasinsBtn.textContent=appContext.debugBasins?'Basins On':'Basins'}if(typeof renderMaterialMenu==='function')renderMaterialMenu()}
function setTool(t){const nextTool=setCurrentTool(t);clearTransientEditUiState();if(nextTool==='eraser'){appContext.running=false;setStatus('Erase: time paused; removes material, tint, and source')}else if(nextTool==='fill'){appContext.running=false;setStatus('Fill: replace one connected region')}else if(nextTool==='force'){appContext.running=false;setStatus('Force: draw an area circle, then an arrow')}else if(nextTool==='color'){setStatus('Color: tint cells without changing material')}else if(nextTool==='source'){setStatus('Source: paint an infinite flow-material generator')}else setStatus('Brush: paint material directly');syncButtons();updateFillPreview(appContext.hoverPoint);renderApp()}
function setSelected(s){setCurrentSelectedKey(s);clearFillPreview();const mat=MATERIAL_FROM_NAME[currentSelectedKey()]||WATER;setStatus(isSolidMaterial(mat)?'Stone: fixed stone is drawn':'Material changed');syncButtons();updateFillPreview(appContext.hoverPoint);renderApp()}
